...
class CCSDSEMReader(object):
    ...
    def Initialize(self):
        r""" Initialize will parse the file, initialize and validate each segment, and check to make sure the segments are ordered correctly by increasing time"""
...
class CCSDSEMSegment(object):
    ...
    def ParseEpoch(epochString):
        r""" Parse a time string read from the EM file and convert it to a Real (A1Mjd) epoch"""
...
class CCSDSAEMSegment(CCSDSEMSegment):
    ...
    def IsRotationInertialToBody(self):
        r""" Returns true of the rotation is inertial-to-body; false if the rotation is body-to-inertial"""
...
class LeapSecsFileReader(object):
    ...
    def IsInLeapSecond(self, theTaiMjd):
        r""" Determines whether or not the input time (in TAI MJD, referenced to GmatTimeConstants::JD_MJD_OFFSET) is in a leap second"""
...
r""" TODO: : DJC - Do we need this for backwards compatibility?"""
...
class Covariance(object):
    ...
    def GetElementIndex(self, elementName):
        return _gmat_py.Covariance_GetElementIndex(self, elementName)
    ...
    def GetElementSize(self, elementName):
        return _gmat_py.Covariance_GetElementSize(self, elementName)

    def GetElementOwner(self, elementName):
        return _gmat_py.Covariance_GetElementOwner(self, elementName)
...
class ElementWrapper(object):
    ...
    def Clone(self):
        r""" todo: Remove implementation in .cpp file, since all it does is throw;       this is the better approach because it FORCES the developers to       implement the method."""
...
class Attitude(GmatBase):
    ...
    def GetEulerAngles(self, *args):
        r"""
        *Overload 1:*
        Method to get the attitude as a set of Euler angles, using the Euler sequence provided by the user (an exception will be thrown if the sequence has not been set

        |

        *Overload 2:*
        Method to get the attitude as a set of Euler angles, using the input Euler sequence
        """
    ...
    def CSModifyAllowed(self):
        r""" Method returning a flag indicating whether or not modification of the reference coordinate system is allowed for this attitude model"""
        return _gmat_py.Attitude_CSModifyAllowed(self)

    def SetInitialAttitudeAllowed(self):
        r""" Method returning a flag indicating whether or not setting of the initial attitude conditions is allowed for this attitude model"""
...
class SpiceAttitude(Attitude):
    ...
    def GetEulerAngles(self, *args):
        r"""
        *Overload 1:*
        get the attitude as a set of Euler angles, using the Euler sequence provided by the user (an exception will be thrown if the sequence has not been set
...
class PhysicalModel(GmatBase):
    ...
    def SetState(self, *args):
        return _gmat_py.PhysicalModel_SetState(self, *args)
...
class FiniteThrust(PhysicalModel):
    ...
    def GetDerivatives(self, state, dt, order, id=-1):
...
class CelestialBody(SpacePoint):
    ...
    def GetStringArrayParameter(self, id):
        return _gmat_py.CelestialBody_GetStringArrayParameter(self, id)
...
class AtmosphereModel(GmatBase):
    ...
    def SetCentralBody(self, cb):
        return _gmat_py.AtmosphereModel_SetCentralBody(self, cb)
...
class DeFile(PlanetaryEphem):
    ...
    def GetStartDayAndYear(self):
        r""" method to return the day-of-year and year of the start time of the DE file."""
        return _gmat_py.DeFile_GetStartDayAndYear(self)

    def Convert(self, deFileNameAscii):
        r""" method to convert an ASCII file to a binary file; this method assumes that there is  an appropriate header file in the same directory as the ASCII file.  TODO: - Implementation is currently TBD"""
...
class CallFunction(GmatCommand):
    ...
    def HasOtherReferenceToObject(self, withName):
        r""" Override this method so that references can be checked for Remove/Delete of objects"""
...
class Spacecraft(SpaceObject):
    ...
    def GetAttitudeRotationMatrix(self, epochGT):
        r""" Rotation matrix from attitude frame to inertial frame (MT matrix)                                        // made changes by TUAN NGUYEN"""
    ...
    def AddExternalStmSetting(self, stmLabel, stmObj, parmIndex):
        return _gmat_py.Spacecraft_AddExternalStmSetting(self, stmLabel, stmObj, parmIndex)
...
class Solver(GmatBase):
    ...
    CREATED = _gmat_py.Solver_CREATED
    COPIED = _gmat_py.Solver_COPIED
    INITIALIZED = _gmat_py.Solver_INITIALIZED
    RUN = _gmat_py.Solver_RUN
    CONVERGED = _gmat_py.Solver_CONVERGED
    EXCEEDED_ITERATIONS = _gmat_py.Solver_EXCEEDED_ITERATIONS
    FAILED = _gmat_py.Solver_FAILED
    UNKNOWN_STATUS = _gmat_py.Solver_UNKNOWN_STATUS
...
class Hardware(GmatBase):
    ...
    def CheckTargetVisibility(self, target):
        return _gmat_py.Hardware_CheckTargetVisibility(self, target)

    def GetMaskConeAngles(self):
        return _gmat_py.Hardware_GetMaskConeAngles(self)

    def GetMaskClockAngles(self):
        return _gmat_py.Hardware_GetMaskClockAngles(self)
...
class LocatedEvent(object):
    ...
    def GetReportString(self):
        r""" Report string - abstract method to return the report string for the event"""
...
class EphemManager(object):
    ...
    def StopRecording(self, done=True):
        r""" Stop recording - load the last ephem data - this must be called at the end of the run for the last-written SPK to be loaded correctly"""
    ...
    def GetContactIntervals(self, observerID, minElevation, obsFrameName, occultingBodyNames, abCorrection, s, e, useEntireIntvl, useLightTime, transmit, stepSize, numIntervals, starts, ends):
        return _gmat_py.EphemManager_GetContactIntervals(self, observerID, minElevation, obsFrameName, occultingBodyNames, abCorrection, s, e, useEntireIntvl, useLightTime, transmit, stepSize, numIntervals, starts, ends)
...
class Interpreter(object):
    ...
    def FindPropertyID(self, obj, chunk, owner, id, type):
        return _gmat_py.Interpreter_FindPropertyID(self, obj, chunk, owner, id, type)
